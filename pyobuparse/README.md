...
1.  Clone the repository:
    ...
    cd pyobuparse
    ```
2.  Navigate into the `pyobuparse` sub-directory (where `pyproject.toml` and `setup.py` for the Python package are located):
    ```bash
    cd pyobuparse 
    ```
3.  Install in editable mode (recommended for development):
    ...
    pip install -e .
    ```
    This will compile the C extension (using `../obuparse.c` and `../obuparse.h` from the repository root) and install the package.
    
    To run tests or build documentation, you might need to install additional dependencies:
    ```bash
...
# For this example, let's use some sample OBU byte strings:

# Minimal Sequence Header OBU (type 1, 11 zero bytes payload)
# OBU Header: 0x0A (type 1, obu_extension_flag=0, obu_has_size_field=1)
# OBU Size (leb128): 0x0b (11 bytes)
sample_seq_header_payload = b"\x00" * 11
sample_seq_header_obu = b"\x0a\x0b" + sample_seq_header_payload

# Minimal Padding OBU (type 15, 1 zero byte payload)
# OBU Header: 0x7A (type 15, obu_extension_flag=0, obu_has_size_field=1)
...
seq_header_payload = b"\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00" # Placeholder
...
The package uses `setuptools` to build the C extension. When you run `pip install .` or `pip install -e .` from the `pyobuparse/pyobuparse` directory, it should automatically compile `../obuparse.c` (relative to `setup.py`). Ensure you have a C compiler and Python development headers installed on your system.

*   On Debian/Ubuntu: `sudo apt-get install build-essential python3-dev`
*   On Fedora: `sudo dnf groupinstall "Development Tools" && sudo dnf install python3-devel`
...
When building on Windows using MSVC, you may observe several `C4244` warnings related to data type conversions (e.g., `'=': conversion from 'uint64_t' to 'int', possible loss of data`). These warnings originate from the `obuparse.c` library. While the Python wrapper aims to function correctly, these warnings indicate potential areas in the C code where data precision *could* be lost if extremely large numerical values are encountered that exceed the capacity of the smaller target types. For typical OBU parsing scenarios, these may not impact functionality, but users performing in-depth analysis or working with unusual AV1 streams should be aware of them. Addressing these warnings would require modifications to the underlying `obuparse.c` code.
...
2.  From the repository root (`pyobuparse/`), run:
    ```bash
...
Detailed API documentation can be built using Sphinx.
1.  Ensure `sphinx` and any required themes/extensions are installed (e.g., `pip install sphinx alabaster`).
2.  Navigate to the `pyobuparse/docs` directory.
3.  Run `make html`.
The generated HTML documentation will be in `pyobuparse/docs/_build/html/index.html`.

Alternatively, refer to the docstrings within the `pyobuparse.parser` and `pyobuparse.cli.obudump_py` modules for API details.
